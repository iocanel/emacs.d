...
In these cases `:ensure nil` needs to be set. Let's create a macro `use-feature`
that combines `use-package` with `:ensure nil` nicely.
...
#+begin_src emacs-lisp
  ...
  (defun my/4k-screen-setup()
    "Modify theme for 4k monitor use"
    (interactive)
    (set-face-attribute 'default nil :font "JetBrains Mono Nerd Font Bold" :height 180)
    (when (boundp 'treemacs-root-face)
      (set-face-attribute 'treemacs-root-face nil :height 210)))
  ...
  (setq my/selected-screen-setup 'my/4k-screen-setup)
  (my/4k-screen-setup)
#+end_src
...
#+begin_src emacs-lisp
  (defun my/toggle-mode-line ()
    "Toggle the mode line on and off."
    (interactive)
    (if mode-line-format
        (progn
      ...
      (setq-default mode-line-format my/saved-mode-line-format))
    (force-mode-line-update t))

  (defvar my/saved-mode-line-format nil)
#+end_src
**** Doom modeline
#+begin_src emacs-lisp
...
******* Code customization
******** Maven
This section introudces some function that are meant to be used for dependency resultion and classpath construction.
...
#+end_src
******** Org Babel Java
In order to support features like `Unamed Class` and `Source Code Execution` we need to customize the `org-babel-execute:java` function.
We need to introduce a few new parameters:
- `:unamed` - if true, the code block is an unamed class
- `:source-exec` - if true, the code block should be executed as source code

We also need to modify the code in order to respoect these parameters.
Specifically, we need for unamed classes to disable code that automatically wraps the block in a class and adds a main method.
For source code execution, we need to modify the actual command to skip the compilation step and to also enable preview features.
...
#+RESULTS:
***** Redis
The code below implements support for using Redis in org-babel code blocks.
The goal is to easily execute simple commands against a Redis server from within an org-mode document
as if the where running from the `redis-cli`.

To further simplify things, the code will automatically start a temporary Redis server if no `:host`, `:port`, or `:auth` is specified.
****** Implementation
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package xwwp
    ...
    :ensure (xwwp :host github :repo "canatella/xwwp" :branch "consult-support")
    :custom
...
***** Prompts
****** Org Mode
#+begin_src text :tangle (concat my/local-dir "llm/prompts/org-mode-creator.txt") :mkdirp t
...
#+begin_src emacs-lisp
  ...
  (use-package codeium
    ...
    (setq codeium/metadata/api_key (replace-regexp-in-string "\n\\'" "" (shell-command-to-string "pass show codeium.com/iocanel/api-key-emacs")))
    :config
